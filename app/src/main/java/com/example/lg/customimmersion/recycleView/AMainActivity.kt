...
class AMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Looper.myQueue().addIdleHandler {
            thread {
                repeat(10) {
                    val linearLayout = LinearLayout(this@AMainActivity).apply {
                        orientation = LinearLayout.VERTICAL
                    }

                    //将itemView add到linearLayout上，后有remove掉，为了正确的将item布局中padding显示出来
                    val itemView = LayoutInflater.from(this@AMainActivity)
    ...
    fun jump(view: View) {
        startActivity(Intent(this, BMainActivity::class.java))
    }
