 ...
 * 1.沉浸式全屏模式
 * 2.状态栏着色模式
 * 由于Android 4.4才加入透明状态栏，Android 5.0之后可以直接设置状态栏和导航栏
 */
public class SystemBarHelper {
    private static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? 0.0f : 0.1f;
    ...
    public static void immersiveStatusBar(Window window, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {//SDK<19(4.4)
            return;
        }
        Log.i("statusBarHeight",""+Build.VERSION.SDK_INT);
        ...
        Log.i("statusBarHeight2",""+rootView.getLayoutParams());
        setTranslucentView(decorView, alpha);
    }
